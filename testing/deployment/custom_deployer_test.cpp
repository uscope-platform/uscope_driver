...
TEST(custom_deployer, deployment) {
    ...
    ASSERT_EQ(control_writes.size(), 41);
    for(int i = 0; i<control_writes.size(); i++){
        ASSERT_EQ(control_writes[i].first, control_writes_addresses[i]);
        ASSERT_EQ(control_writes[i].second, control_writes_data[i]);
    }
}