...
nlohmann::json custom_deployer_get_addr_map(){
    ...
    uint64_t waveform_generator = 0x4'43c9'0000;
    ...
    bases["waveform_generator"] = waveform_generator;
...
TEST(custom_deployer, deployment) {
    ...
    for(int i = 0; i<ops.size()-2; i++){
        ASSERT_EQ(ops[i+2].type, control_plane_write);
